...
MODEL_PATH = Path("models_TFDS_CD/cats_vs_dogs_mobilenetv2.keras")
CLASS_NAMES_PATH = Path("models_TFDS_CD/class_names.txt")
OUTPUT_DIR = Path("predict_outputs_TFDS_CD")
...
def save_prediction_result(
    ...
    save_path = OUTPUT_DIR / f"{image_path.stem}_tfds_catdog_prediction.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="TFDS cats_vs_dogs MobileNetV2 모델로 고양이/개 이미지를 예측합니다."
    )
    ...
    if not MODEL_PATH.exists():
        raise FileNotFoundError(
            "모델 파일이 없습니다. 먼저 `python src\\05_train_cats_vs_dogs.py`를 실행하세요."
        )
